...
- Passing std-JIT smoke tests (current):
  ...
  - `std_jit_vec_new_forget_smoke` (passes under `--run-ignored only`; currently kept as probe)
...
- `just test-clif std_jit_vec_new_forget_smoke --run-ignored only --no-capture` passes
...
2. Unignore probes that now pass to keep regressions visible.
   - Candidate unignores: `std_jit_string_from_smoke`, `std_jit_vec_new_forget_smoke`.

3. Re-run and unignore probes incrementally as each blocker is fixed.
   - Current state: `str_parse` and `vec_push` are unignored; `string_from` now passes but is still marked ignored.
