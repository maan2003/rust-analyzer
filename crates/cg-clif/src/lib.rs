...
fn codegen_intrinsic_call(
    ...
    let result = match name {
        ...
        "slice_get_unchecked" => {
            assert_eq!(args.len(), 2, "slice_get_unchecked expects 2 args");

            let slice_ptr = codegen_operand(fx, &args[0].kind);
            let mut index = codegen_operand(fx, &args[1].kind).load_scalar(fx);

            let data_ptr = match slice_ptr.layout.backend_repr {
                BackendRepr::ScalarPair(_, _) => slice_ptr.load_scalar_pair(fx).0,
                BackendRepr::Scalar(_) => slice_ptr.load_scalar(fx),
                _ => panic!(
                    "slice_get_unchecked expects pointer or fat-pointer layout, got {:?}",
                    slice_ptr.layout.backend_repr,
                ),
            };

            if fx.bcx.func.dfg.value_type(index) != fx.pointer_type {
                index = codegen_intcast(fx, index, fx.pointer_type, false);
            }

            let slice_ptr_ty = operand_ty(fx.db(), body, &args[0].kind);
            let pointee_ty = slice_ptr_ty
                .as_ref()
                .builtin_deref(true)
                .expect("slice_get_unchecked first argument must be a pointer/reference");

            let elem_size = match pointee_ty.kind() {
                TyKind::Slice(elem_ty) | TyKind::Array(elem_ty, _) => fx
                    .db()
                    .layout_of_ty(elem_ty.store(), fx.env().clone())
                    .expect("slice_get_unchecked element layout")
                    .size
                    .bytes(),
                TyKind::Str => 1,
                _ => panic!("slice_get_unchecked pointee must be [T]/str, got {pointee_ty:?}"),
            };

            let byte_offset =
                if elem_size == 1 { index } else { fx.bcx.ins().imul_imm(index, elem_size as i64) };
