...
pub(crate) fn detect_variant_from_bytes<'a>(
    ...
    let (var_id, var_layout) = match &layout.variants {
        ...
        hir_def::layout::Variants::Single { index } => {
            (e.enum_variants(db).variants[index.as_usize()].0, layout)
        }
        hir_def::layout::Variants::Multiple { tag, tag_encoding, variants, .. } => {
            ...
            match tag_encoding {
                TagEncoding::Direct => {
                    let (var_idx, layout) =
                        variants.iter_enumerated().find_map(|(var_idx, v)| {
                            let def = e.enum_variants(db).variants[var_idx.as_usize()].0;
                ...
                TagEncoding::Niche { untagged_variant, niche_start, .. } => {
                    ...
                    (e.enum_variants(db).variants[variant.as_usize()].0, &variants[variant])
                }
            }
        }
    };
